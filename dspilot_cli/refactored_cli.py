...
class DSPilotCLI:
    """
    DSPilot CLI 메인 클래스 (리팩토링됨)
    
    이제 단일 책임 원칙을 따르며, 각 기능이 별도 클래스로 분리됨:
    - OutputManager: 출력 관리
    ...
    def __init__(self, debug_mode: bool = False, quiet_mode: bool = False, 
        ...
        self.interaction_manager = InteractionManager(self.output_manager, full_auto_mode)
    ...
    async def _process_query(self, user_input: str) -> None:
        ...
        self.output_manager.log_if_debug(f"=== CLI: 대화형 Agent 처리 시작: '{user_input}' ===")
        self.output_manager.print_system(Messages.ANALYZING)

        try:
            await self._run_interactive_agent(user_input)
        except Exception as e:
            self.output_manager.log_if_debug(f"=== CLI: 대화형 Agent 처리 실패: {e} ===", "error")
    ...
    async def _run_interactive_agent(self, user_input: str) -> None:
        ...
        enhanced_prompt = self.conversation_manager.build_enhanced_prompt(user_input)
    ...
    async def _display_response(self, response_data: dict) -> None:
        ...
        self.conversation_manager.add_to_history("assistant", response, {"used_tools": used_tools})
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
사용 예시:
  python -m application.cli.refactored_cli                          # 대화형 모드 (도구 사용 시 사용자 확인)
  python -m application.cli.refactored_cli --full-auto              # 대화형 모드 (도구 자동 실행)
  python -m application.cli.refactored_cli "현재 시간은?"             # 단일 질문 (간결 출력)
  python -m application.cli.refactored_cli "현재 시간은?" --full-auto # 단일 질문 (자동 실행)
  python -m application.cli.refactored_cli "현재 시간은?" --debug     # 단일 질문 (상세 로그)
  python -m application.cli.refactored_cli --tools                  # 도구 목록
        """
    )
