...
class NotificationHandler(BaseHandler):
    ...
    async def send_system_notification(self, request: SystemNotificationRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            # 대화창에도 메시지 추가
            notification_content = f"**[시스템 알림] {request.title}**\n\n{request.message}"
            self.notification_signals.add_api_message.emit("system", notification_content)
    ...
    async def send_dialog_notification(self, request: DialogNotificationRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            # 대화창에도 메시지 추가
            message_content = request.html_message or request.message or "다이얼로그 알림"
            notification_content = f"**[다이얼로그] {request.title}**\n\n{message_content}"
            self.notification_signals.add_api_message.emit("notification", notification_content)
    ...
    async def send_html_dialog(self, request: DialogNotificationRequest) -> Dict[str, Any]:
        """HTML 다이얼로그 알림 전송"""
        try:
    ...
    async def send_info_notification(self, request: NotificationRequest) -> Dict[str, Any]:
        """정보 알림 전송"""
        return await self._send_notification_by_type("info", request)

    async def send_warning_notification(self, request: NotificationRequest) -> Dict[str, Any]:
        """경고 알림 전송"""
        return await self._send_notification_by_type("warning", request)

    async def send_error_notification(self, request: NotificationRequest) -> Dict[str, Any]:
        """오류 알림 전송"""
        return await self._send_notification_by_type("error", request)

    async def send_confirm_notification(self, request: NotificationRequest) -> Dict[str, Any]:
        """확인 알림 전송"""
        return await self._send_notification_by_type("confirm", request)

    async def send_auto_notification(self, request: NotificationRequest) -> Dict[str, Any]:
        """자동 알림 전송"""
    ...
    async def _send_notification_by_type(
        ...
        try:
            self._log_request(f"send_{notification_type}_notification", request.model_dump())
            ...
            return self._create_error_response(f"{notification_type} 알림 전송 실패", exception)

    # 레거시 호환성을 위한 메서드
    async def send_notification_legacy(self, notification: NotificationMessage) -> Dict[str, Any]:
        """
