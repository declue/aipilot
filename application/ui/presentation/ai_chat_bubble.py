...
class AIChatBubble(ChatBubble):  # pylint: disable=too-many-ancestors
    ...
    def set_reasoning_info(
        self, is_reasoning: bool, reasoning_content: str = "", final_answer: str = ""
    ) -> None:
        ...
        if is_reasoning:
            logger.debug(
                f"추론 모델 감지: 추론 {len(reasoning_content)}자, 답변 {len(final_answer)}자"
            )
    ...
    def _render_reasoning_content(self) -> None:
        ...
        # 스트리밍 중인지 확인 (스트리밍 중이면 열린 상태로 표시)
        is_streaming = getattr(self, 'is_streaming', False)
        details_state = "open" if is_streaming else ""
        
        # 스트리밍 상태에 따른 아이콘과 텍스트 변경
        if is_streaming:
            status_icon = "🤔"
            status_text = "추론 중..."
            cursor = '<span style="color: #10B981; font-weight: bold; margin-left: 4px;">▌</span>'
        else:
            status_icon = "✅" if self.final_answer else "🤔"
            status_text = "추론 완료" if self.final_answer else "추론 과정"
            cursor = ""
    ...
    def _render_normal_content(self) -> None:
        ...
        # 표시할 내용 결정
        content = (
            self.final_answer if (self.is_reasoning_model and self.final_answer) else self.message
        )
            ...
            def adjust_height() -> None:
                try:
                    ...
                    if browser.height() != final_height:
                        ...
                        logger.debug(
                            f"텍스트 브라우저 높이 조절: {final_height}px (문서 높이: {doc_height}px)"
                        )
