...
from .interfaces import ConfigDict, ConfigValue, ConfigType


def get_nested_value(data: ConfigDict, key: str, fallback: Optional[ConfigValue] = None) -> ConfigValue:
...
def flatten_config(data: ConfigDict, separator: str = '.', prefix: str = '') -> Dict[str, ConfigValue]:
    """중첩된 설정을 평면화
...
def validate_config_structure(data: ConfigDict, required_keys: List[str], optional_keys: Optional[List[str]] = None) -> List[str]:
    """설정 구조 검증
    ...
    for key in flattened.keys():
        if key not in all_allowed_keys:
            # 부분 매칭도 확인 (중첩 구조 고려)
            is_allowed = any(key.startswith(allowed + '.') or allowed.startswith(key + '.') 
