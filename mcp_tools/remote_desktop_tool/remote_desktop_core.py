#!/usr/bin/env python3
"""
Remote Desktop 핵심 기능들 (FastMCP 없이 순수 함수)
테스트 및 독립 실행용
"""
...
if __name__ == "__main__":
    """간단한 테스트"""
    print("🖥️  Remote Desktop Core Functions Test")
    print("=" * 50)
    ...
    if capture["success"]:
        print(f"✅ 성공: {capture['width']}x{capture['height']}")
        # 이미지 크기 확인 (KB)
        image_size = len(capture["image"]) * 3 / 4 / 1024  # base64는 원본의 4/3 크기
        print(f"📊 이미지 크기: {image_size:.1f} KB")
    else:
        print(f"❌ 실패: {capture['error']}")
